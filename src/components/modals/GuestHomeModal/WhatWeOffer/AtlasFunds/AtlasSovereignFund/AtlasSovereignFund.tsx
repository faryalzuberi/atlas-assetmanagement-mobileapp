...
import {getSovereignFund} from '../../../../../../config/api/whatWeOffer';
import { Text } from 'react-native-svg';


export const AtlasSovereignFund = () => {
  ...
  const [sovereignFundList, setSovereignFundList] = useState<any>([]);
  useEffect(() => {
    if (data?.data?.data ) {
      let json: any = [];
  ...
  return (
    <>
      ...
      <DetailList
        arrayList={sovereignFundList}
        interval={1}
        headerTitle={languageTxt.atlasSovereignFundTxt}
      />
    </>
  );
};
