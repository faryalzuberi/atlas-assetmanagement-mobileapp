...
import {getAtlasStockMarketFund} from '../../../../../../config/api/whatWeOffer';



export const AtlasStockMarketFund = () => {
  const {isLoading, error, data, refetch}: any = useQuery(
    'getAtlasStockMarketFund',
    getAtlasStockMarketFund,
    {select: (boardOfDirectors: any) => boardOfDirectors},
  );

  const [stockMarketFundList, setStockMarketFundList] = useState<any>([]);
  useEffect(() => {
    if (data?.data?.data ) {
      let json: any = [];
  ...
  return (
    <>
      ...
      <DetailList
        arrayList={stockMarketFundList}
        interval={1}
        headerTitle={languageTxt.atlasStockMarketFundTxt}
      />
    </>
  );
};
